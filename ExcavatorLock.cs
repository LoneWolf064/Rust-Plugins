...
{
    [Info("Excavator Lock", "Lorenzo", "0.4.35")]
...
#region Localization
        ...
        object OnDieselEngineToggle(DieselEngine engine, BasePlayer player)
        {
            ...
            if (!Associate.TryGetValue(engine.net.ID.Value, out armID)) return true;  // 
        ...
        object OnExcavatorGather(ExcavatorArm arm, Item item)
        {
            ...
            {
                if (item.info.shortname == "sulfur.ore" && _config.SulfurMult != 0f) item.amount = (int)((float)item.amount * _config.SulfurMult);
                else if (item.info.shortname == "hq.metal.ore" && _config.HQMMult != 0f) item.amount = (int)((float)item.amount * _config.HQMMult);
                else if (item.info.shortname == "metal.fragments" && _config.MetalMult != 0f) item.amount = (int)((float)item.amount * _config.MetalMult);
                else if (item.info.shortname == "stones" && _config.StoneMult != 0f) item.amount = (int)((float)item.amount * _config.StoneMult);
            }
...
#region Helpers
        ...
        {
            if (_config.Debug)
                UnityEngine.Debug.Log($"[{_instance.Name}] [{DateTime.Now.ToString("h:mm tt")}] " + message);
        ...
        internal void Trace(string message, int indentation = 0) => LogToFile(TraceFile, $"[{DateTime.Now.ToString("h:mm tt")}] " + "".PadLeft(indentation, ' ') + message, this);
        ...
        private void PrintToDiscord(string message, double seconds = 0)
        {
            ...
            if (_discord.MsgCooldown <= DateTime.Now.ToBinary())
            {
				...
				if (_config.UseDiscordTimestamp)
					_discord.SendTextMessage($"[<t:{unixTime}:t>] " + message);
				else
			        _discord.SendTextMessage($"[{DateTime.Now.ToString("h:mm tt")}] " + message);
            }
        }
...
#region Shared.Components
        ...
        {
            private const float PostDelay = 1.0f; // set 2 sec min delay between two discord post
